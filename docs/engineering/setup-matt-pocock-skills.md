...
- [wayfinder](https://aihero.dev/skills-wayfinder)'s two labels — `wayfinder:map` and `wayfinder:answers` — are not created here either, and `gh issue create --label <missing>` fails outright rather than creating the label. Create both by hand before the first wayfinder run on a GitHub repo.
