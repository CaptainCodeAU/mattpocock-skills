...
| A [wayfinder](https://aihero.dev/skills-wayfinder) map has cleared | `/to-spec <map>` — the issue number on a tracker, or the path to `map.md` on local markdown |
...
The map, not the answer key — `/to-spec #<map_issue>` on a tracker, or the path to `map.md` on local markdown. The map links its **answer key**, which is where the decisions actually live, and `to-spec` follows that link. [wayfinder](https://aihero.dev/skills-wayfinder) produces decisions rather than deliverables; `to-spec` is the step that collapses them into one buildable document. Looping the map straight into `/implement` throws that collapse away.
